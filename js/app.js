...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
        <div class="single-product"> <br>
          <img class="product-image w-50" src=${product.image}> </img>          
          <div class="card-body">
                <h4> ${product.title} </h4>
                <p>Category: ${product.category}</p>
                <h4>Price: $ ${product.price}</h4>
                <h6><br>Average Rating: ${product.rating.rate} <i class="fas fa-star-half-alt fs-4 text-warning my-3"></i> <br> Rated By: ${product.rating.count} </h6>
